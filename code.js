...
function permutationSort(a) {
    ...
    function generatePermutations(array, start, end, checkedPermutationsCount) {
        // If we've gone beyond the end of the array, increment the count and return
        if (start > end) {
            return checkedPermutationsCount + 1; 
        }
        ...
        for (let i = start; i <= end && count === 0; i++) {
            ...
            count += generatePermutations(array.slice(), start + 1, end, checkedPermutationsCount); // Recur for the rest of the array
  ...
  console.log(`Number of permutations checked until sorted one is found: ${permutationCount}`);


// I am still working on this.
// There are errors in the number of permutations checked and the log of the current permutation being checked. 
